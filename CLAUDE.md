...
`QueryChannel` draws a careful three-way distinction — "queried and found nothing" (`{ ok: true, data: null }` / `{ ok: true, data: [] }`), "no live instance to even ask" (`no-live-instance`), "scene attached but no physics world" (`no-physics-world`) — and every tool that relays through it preserves that shape exactly rather than collapsing any of the three into "empty." A query the connected client never answers within 5 seconds (`QUERY_TIMEOUT_MS` in `lib/bridge.ts`) also resolves `no-live-instance`, never a fabricated empty result — a stalled reply and "nothing to ask" both mean the caller can't trust an answer, so they get the same honest error.

`QueryChannel`'s `EngineQuery` union was extended engine-side to add `createEntity`, `actorSendMessage`/`actorBroadcast`/`actorInboxSize`/`actorList`, and `navmeshFindPath`/`navmeshNearestNode` (see `emptysock-engine`'s `RELEASE_PASS.md` and `packages/engine/src/bridge/QueryChannel.ts`), and `lib/bridgeTypes.ts` mirrors all seven query kinds plus their two new error codes (`no-actor-system`, `no-navmesh`). `navmesh_*`, `actor_*`, and `scene_create_entity` are now all live — they relay real queries and preserve the same honest distinction their domain adds: `no-live-instance` (nothing connected), `no-actor-system`/`no-navmesh` (connected, but that scene has no `ActorSystem`/navmesh attached — a scene can legitimately have neither, so this is never `no-live-instance`), and a real empty/null result (`path: null`, zero actors) when the query genuinely found nothing.

`scene_create_entity` relays to `createEntity`, which spawns a bare entity and reports any unresolvable component names back in `skipped` rather than failing the whole call — the same "partial success, honestly reported" shape `SaveSystem`'s migration failures use.
